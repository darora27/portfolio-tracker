...
async function run() {
  ...
  const resp = await page.request.post(`${BASE}/api/auth/login`, { data: { password: PASSWORD } });
  const cookies = await context.cookies();
  const cookie = cookies.find((c) => c.name === "owner_session");
  await context.close();

  const results = [];
  // Test each specific ticker (by direct attribute selector, not .first()) navigates correctly.
  for (const targetTicker of ["ASML", "GOOG", "CRM", "MSFT"]) {
    ...
    const locator = p.locator(`[data-radar-ticker="${targetTicker}"]`).first();
    const exists = await locator.count();
    if (!exists) { results.push({ targetTicker, exists: false }); await ctx.close(); continue; }
    await locator.dblclick({ force: true });
    await p.waitForTimeout(300);
    results.push({ targetTicker, navigatedTo: p.url() });
    await ctx.close();
  }
