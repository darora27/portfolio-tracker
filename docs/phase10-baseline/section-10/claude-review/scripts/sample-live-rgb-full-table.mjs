...
const samples = descriptors.map((descriptor) => {
  ...
  result.deltaFail = result.deltaE > 8;
...
for (const direction of [1, -1]) {
  ...
  for (let index = 1; index < sameDirection.length; index += 1) {
    ...
    const ordered = direction > 0
      ...
      : current.luminance <= prior.luminance;
    if (!ordered) {
      console.log("ORDERING_FAIL", JSON.stringify({dir:direction, prior:prior.ticker, current:current.ticker, priorL:prior.luminance, curL:current.luminance}));
    }
  }
}
