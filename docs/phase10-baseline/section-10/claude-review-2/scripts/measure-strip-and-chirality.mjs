...
// Non-throwing full table for the two assertions inside the section's own
// docs/phase10-baseline/section-10/scripts/capture-live-sphere-strip.mjs:
//   1. equatorial-band mean relative luminance in [0.16, 0.55]   (VIS-01)
//   2. mark chirality: normalScore > mirroredScore                (DEF-02)
// The measurement code below is copied verbatim from that verifier so the
// numbers are directly comparable; only the throws are replaced by records.
  ...
  JSON.stringify(
    {
      viewport: "1440x900",
      base,
      luminanceWindow: [0.16, 0.55],
      measurements,
      luminanceFailures: measurements.filter((m) => !m.inWindow).map((m) => m.ticker),
      chirality,
      chiralityFailures: chirality.filter((c) => !c.pass).map((c) => c.ticker),
    },
    null,
    1,
  ),
