...
// §11): ready once every holding has painted its projected planet centre
// and radius — the samples below aggregate over a further settle window to
// smooth frame-to-frame jitter, not to wait for mount.
...
try {
  ...
  const criterion1_allLabelsInsideViewport = rows.every((r) => r.labelFullyInsideViewport);
  const criterion17_allPlanetsInsideViewport = rows.every((r) => r.planetFullyInsideViewport);
  const measured = {
    url: base,
    viewport: { width: viewportWidth, height: viewportHeight },
    sampleCount: samples.length,
    labelCount: rows.length,
    criterion1_allLabelsInsideViewport,
    criterion17_allPlanetsInsideViewport,
