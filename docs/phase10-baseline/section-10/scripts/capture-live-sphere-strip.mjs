...
// from, both at overview and per-holding approach.
...
async function waitForApproachReady(page, ticker) {
  await page.locator("canvas").waitFor({ state: "visible" });
  await page.waitForFunction(
    (t) => document.querySelector(`[data-scene-ticker="${t}"][data-planet-center-x]`) !== null,
    ticker,
  );
  await page.waitForTimeout(1_500);
}
...
try {
  ...
  for (const descriptor of descriptors) {
    ...
    await waitForApproachReady(page, descriptor.ticker);
  ...
  emit(CRITERION, chirality.length === 8 && chirality.every(({ pass }) => pass), measured);
} catch (error) {
