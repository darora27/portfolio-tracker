...
// moons, satellites, belt bodies) are attached synchronously with the
// canvas mount, so no scene-ticker wait is required here, only a short
// settle window for the first animation frame.
...
async function waitForMissionControlReady(page) {
  ...
  await page.waitForFunction(
    () => document.querySelectorAll("[data-radar-ticker]").length > 0,
  );
}
...
try {
  ...
  const expectedTabOrder = await page.evaluate(() => {
    ...
    const beltBodies = [...document.querySelectorAll("[data-belt-holding]")].map(
      (element) => `belt-body:${element.dataset.beltHolding}`,
    );
    return [
      "sun",
      ...planets,
  ...
  for (let index = 0; index < expectedTabOrder.length; index += 1) {
    ...
    tabWalk.push(
      await page.evaluate(() => {
        ...
        else if (element.dataset.beltHolding) {
          id = `belt-body:${element.dataset.beltHolding}`;
        }
  ...
  const firstRadarTicker = await page
    .locator("[data-radar-ticker]")
    .first()
    .getAttribute("data-radar-ticker");
  if (!firstRadarTicker) throw new Error("No radar ticker target found.");
  const firstRadarRing = page.getByRole("button", {
    name: new RegExp(`^${firstRadarTicker} radar ring`),
  });
  await firstRadarRing.focus();
  await page.keyboard.press("Space");
  await page
    .locator(`[data-manifest-ticker="${firstRadarTicker}"][aria-expanded="true"]`)
    .waitFor();
  const radarExpandedByKeyboard = true;

  await page.goto(`${base}?focus=portfolio&camera=command&station=plot`, {
    waitUntil: "domcontentloaded",
  });
  await waitForMissionControlReady(page);
  const manifestRow = page.locator(
    `[data-manifest-ticker="${firstRadarTicker}"]`,
  ).first();
  ...
  await page
    .locator(`[data-manifest-ticker="${firstRadarTicker}"][aria-expanded="true"]`)
    .waitFor();
  const manifestExpandedByKeyboard = true;
  ...
  await page.waitForURL(
    (url) =>
      url.searchParams.get("holding") === firstRadarTicker &&
      url.searchParams.get("camera") === "approach",
  );
  const radarEnterDestination = page.url();
  ...
  const destinations = {
    ...
    radar: {
      ticker: firstRadarTicker,
      expandedByKeyboard: radarExpandedByKeyboard,
      manifestExpandedByKeyboard,
      enterDestination: radarEnterDestination,
    },
  };
