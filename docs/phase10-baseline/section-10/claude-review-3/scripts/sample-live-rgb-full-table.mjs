...
const root = path.resolve("docs/phase10-baseline/section-10/claude-review-3");
...
const samples = descriptors.map((descriptor) => {
  ...
  result.deltaEPass = result.deltaE <= 8;
  result.hueLockPass =
    anchor === null ? null : !(result.chroma <= 0.3 || result.hueDistance > 10);
  return result;
});
  ...
  for (let index = 1; index < sameDirection.length; index += 1) {
    ...
    if (!ordered) {
      orderingViolations.push({
        direction,
        prior: { ticker: prior.ticker, weekly: prior.weekly, luminance: prior.luminance },
        current: { ticker: current.ticker, weekly: current.weekly, luminance: current.luminance },
      });
    }
  }
}
