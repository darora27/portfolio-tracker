/**
 * Phase 10 §7 Turn B′ review re-measurement — an unmodified copy of
 * measure-desktop.mjs (§2.3.2) except for its OUTPUT path, run by Claude
 * Lead against the remediated /dev/phase10-spike-r3f-world to independently
 * verify criterion 41 (the route-owned long task) after Codex's described
 * optimisation round. Writes to raw/desktop-scene-turn-bprime.json rather
 * than overwriting the Turn B FAIL evidence at raw/desktop-scene.json.
 *
 ...
 *
 *   OWNER_PASSWORD=<temporary-local-value> node docs/phase10-spike-section-7/measure-desktop-turn-bprime.mjs
 *
...
const OUTPUT = resolve(HERE, "raw/desktop-scene-turn-bprime.json");
