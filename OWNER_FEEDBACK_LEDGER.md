...
| FB-36 | **The app takes a while to load** — Jul 31, noticed on the deployed site | 1 | **open · measured.** `public/` is **26 MB**, almost all planet textures: `nbis-base.ktx2` and `msft-base.ktx2` at 3.5 MB each, five more at 2.5 MB, `ibm-base.ktx2` at 1.6 MB. A visitor downloads roughly **20 MB** before the universe looks right. This is now the first impression on a live resume link, so it matters more than it did yesterday. | **The progressive path is half-built already and was never wired.** `public/textures/planets/thumbs/` holds 32px versions of every planet, generated by the texture pipeline — and `grep` finds ZERO references to `thumbs` or `-32.png` anywhere in `src/`. Show the 32px texture immediately, swap to full resolution when it arrives. If that is not enough, the base maps are also larger than a 44–64px overview planet can possibly show; a mid-tier at ~512px would cut the payload by most of an order of magnitude without touching approach-scale quality. **Do not simply re-run the texture generator** — that path consumed five rounds in §11 and the standing rule against it still holds; this is a loading-strategy change, not a texture-content change. | **§13 or §16** | a first-time visitor sees planets in a couple of seconds on a normal connection |
