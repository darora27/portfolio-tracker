...
| FB-13 | Full analysis of an individual stock — the Chart Room | **3** | **designed · SHIPS AS STAGE TWO OF THE SAME SECTION.** `MISSION_CONTROL_ARCHITECTURE.md` §7: Mission Control is the portfolio-manager page, the Chart Room is the stock page, and rows / rings / `FULL ANALYSIS ▸` are the doors. Explicitly bound into the rewiring section **so it cannot slip again behind content work** | Round 6 §1 + the owner-reviewed mock. `UNIVERSE_STOCK_LAB_2.html` is still not in the repo | **next section, stage two** | both doors work; he uses it and says so |
...
| FB-22 | Random yellow semi-circle haze above the sun. *"random yellow semi circle haze thing above the sun"* | 1 | **open · defect** | a rendering artefact he can see; find it before designing anything around it | **§13** | it is gone from a capture |
...
| FB-31 | Orange tabs unnecessary. *"The tabs in orange are not necessary and can be removed."* | 1 | **open** | remove | **§13** | gone from a capture |
