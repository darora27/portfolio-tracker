...
|---|---|---|---|---|---|---|
| FB-01 | Planets too close — **and zoom the whole system out.** Jul 29: *"Spread them out more and zoom out the entire solar system as a whole."* | **4** | **designed · numbers fixed by owner** | **Two levers, both required.** (1) Spacing: audit §5.1 — radii [0.62, 1.35], gap 1.75×(rᵢ+rᵢ₊₁)+0.55. (2) Camera pull-back: `OVERVIEW_BELT_SPAN_PCT` **0.88 → 0.80** in `src/lib/observatory/scene-model.ts`. **Before-evidence, measured by Opus in Chrome on the live app:** sun and ASML edges ~**5px** apart against a required ~68px (one larger-disc radius) — off by more than 13×; bottom third of the frame empty. | **§12a Phase B** | measured gap ≥ 1.0× larger disc, whole system fits with margin, **and he says it looks right** |
...
| FB-17 | Planet panel width — *"still too narrow and needs to be greater in width"*, growing **leftward**, with *"a little bit more information, but simple"* | **4** | **variant strip respecified by owner.** Every correction since 380 has been upward, so the ladder now **starts above 580**: capture **600 / 660 / 720px**. | Panel grows LEFT into the dead space (pairs with the now-closed FB-07 anchor). **Planet-visibility invariant stays asserted at every width** — `raw-panel-geometry.json` must keep `unoccluded: true`, i.e. disc right edge ≤ panel left edge. A width that hides the planet is not a candidate. | **§12a Phase C** | he picks a width from the capture strip |
