...
| FB-04 | *"No company logos are visible on any planet"* — Jul 29 post-drop, again: *"Still not seeing any logos on the planets themselves."* | **6** | **blocked · TEXTURE-PIPELINE DEFECT measured in §11.R.** Direct decode of all eight shipped maps proves six exact mark composites, but IBM/NBIS SVG alpha is an opaque rectangle (100% / 99.5211% mask coverage), so no distinguishable mark reaches those textures. Per his amendment: stopped, reported, **no regeneration**. Evidence: `raw-shipped-mark-measurement.json` + `all-eight-texture-marks.png` | Owner decision on a bounded IBM/NBIS alpha-pipeline repair; approach camera/exposure source work is retained, overview identity stays colour+silhouette | **Devan decision, then live capture** | one capital reads on every *selected* planet in capture; overview policy gets his sign-off |
| FB-05 | Small fonts. Jul 29 post-drop, **after the five-token ramp shipped**: *"Mission control has a lot of font that is too small."* | **4** | **designed · ROOT-CAUSED in §11.R, not fixed.** The ramp reached the stylesheet, but its gate constrains legal values rather than semantic roles; Mission Control still maps plot/manifest headers, question copy, radar labels/detail, rail stations, strip copy, plot labels, and holdings headers to the smallest 11px token. Evidence: `docs/phase10-baseline/section-11/raw-mission-control-type-root-cause.md` | remap Mission Control reading copy to the existing body/title tokens while true labels remain labels — no sixth size, no individual nudges | **§12a, first type item** | he reads Mission Control without squinting |
...
| FB-07 | Planet still too far right. Jul 29 post-drop: *"the planet still shows to be too far over to the right. Needs to be farther over to the left as there is a lot of empty space on the left."* | **2** | **landed in source · NEEDS CAPTURE.** Deterministic projection holds x ≈ 30% from four orbit directions; Chromium was denied before page launch, so this is not visually closed | Round 5 left-third anchor x ≈ 30% | **Devan capture** | measured disc centre ≈ 30% at 1440×900 **and his confirm** |
...
| FB-17 | Planet panel size **and direction of growth.** Jul 29 post-drop, after 460px: *"The planet terminal dashboard thing that comes up should take up more space as well, expanding horizontally to the left. Contain a little bit more information, but simple information."* | **3** | **variant mechanism landed · NEEDS CAPTURE.** The right-anchored rail now accepts 460/520/580px and therefore grows left; no width is chosen without his eyes | pair with FB-07: capture all three widths and let him pick rather than guessing a fourth number | **Devan capture** | he picks a width from a capture **and** the extra fields are simple |
