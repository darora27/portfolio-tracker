...
| FB-33 | CORRELATION and EARNINGS do not earn their place | 1 | **designed — with the orphaned question named.** `MISSION_CONTROL_ARCHITECTURE.md` §5 retires the CORRELATION section; *"do my stocks move as one bet?"* survives as the TOP-2 concentration line and the Chart Room's `MOVES WITH` bars. §6 rules EARNINGS **demote-and-relocate, never delete** — a strip chip, a T−nD chip per HOLDINGS row, and the full picture in the Chart Room | `MISSION_CONTROL_ARCHITECTURE.md` §5–§6 | **next section** | **needs one sentence from him: "earnings as chips instead of a section: yes" or no** |
