...
| FB-28 | **Mission Control content — the old dashboard's substance** | 1 | **open · CRITICAL — and the shape of the fix just changed.** He said content *"keeps getting worse and worse"*. **Nothing was deleted.** An inventory of the tree found **26 dashboard components and 3 history components still present, with `/dashboard`, `/history` and `/trades` routes live** — and Mission Control importing exactly one *type* (`ChartPoint`) and not a single content component from them. Mission Control was built as a parallel surface, so it never inherited what already worked. **This is a re-wiring job, not a rebuild.** | Named by him, already built: `ExcessReturns` / `ValueChart` (portfolio vs XLK/VOO/VTI), `HoldingsPerformanceChart` (per-stock with toggles). Also sitting unused: `CompositionDonut`, `ContributionChart`, `ConcentrationMeter`, `BetaTable`, `HoldingRiskTable`, `CorrelationHeatmap`, `WinnersLosers`, `RealizedUnrealized`, `RiskPanel`, `EarningsCalendar`, `PositionsTable`, `ClassificationBarList`, `MetricDisclosure`, `WhyMode` / `AttentionMode` / `HowAmIDoingMode` / `AllAnalyticsView` | **§12b — BLOCKED ON FB-34** | Mission Control carries the substance he misses and he says so |
...
| FB-35 | **Pie chart of holdings by percentage, and everything from the History tab** | Jul 30: *"on the mission control center I want to see the pie chart of the stock breakdowns by percentage and everything that was in the history tab of the other versions of this application."* | 1 | **open — and all of it already exists, unwired.** `CompositionDonut.tsx` is the pie/donut breakdown. The History tab's contents are `CompositionOverTimeChart.tsx`, `DailyReturnsChart.tsx` and `DrawdownChart.tsx`, all live at `/history`. None is reachable from Mission Control | wire the existing components rather than rebuilding them; they already read the real snapshot data. Part of FB-28's re-wiring | **§12b, with FB-28** | he sees the pie chart and the history content inside Mission Control |
